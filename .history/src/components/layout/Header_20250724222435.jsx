...
import { IoPerson, IoLogOut, IoLogIn, IoMenu, IoClose, IoNotifications } from 'react-icons/io5'; // Corrected Ionicons import
    ...
    <header className="bg-white shadow-sm sticky top-0 z-40 w-full">
      <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex items-center relative">
        ...
        <div className="ml-auto relative">
          <button 
            onClick={toggleMenu}
            className="rounded-full bg-[#5fd1ee] p-2 hover:bg-[#3c73a7] transition-colors focus:outline-none"
            aria-label="User menu"
          >
            <IoPerson size={20} color="#0c28a3" />
          </button>
          ...
          {showMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-200">
              {user ? (
                ...
                >
                  <IoLogOut size={16} color="#0c28a3" className="mr-2" />
                  <span className="text-[#0c28a3] font-medium">Logout</span>
                </button>
              ) : (
                <button
                ...
                >
                  <IoLogIn size={16} color="#000000" className="mr-2" />
                  <span className="text-[#000000] font-medium">Sign In</span>
                </button>
              )}
            </div>
          )}
